...
@app.get("/posts")
async def posts(limit: int | None = None, current_user: models.User = Depends(get_current_user)):
    ...
    data = r.json()
    items = [{"id": x["id"], "title": x["title"], "body": x["body"]} for x in data]
    return JSONResponse(items)


@app.post("/invert-image", response_class=Response)
async def invert_image(file: UploadFile = File(...), current_user: models.User = Depends(get_current_user)):
    try:
        raw = await file.read()
...
def get_page(page_id: int, current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def get_all_kpi(current_admin: models.User = Depends(get_current_admin), db: Session = Depends(get_db)):
    rows = (
        db.query(
